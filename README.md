...
If the web service is created manually in Render, create or connect a PostgreSQL database and add this environment variable before deploying:
...
The build intentionally stops when `DATABASE_URL` is missing on Render. This prevents student accounts and other records from being saved into temporary SQLite storage and disappearing after a restart.
