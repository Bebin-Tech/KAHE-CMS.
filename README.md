...
Use the included `render.yaml` blueprint when creating the Render service. For production, connect a persistent database through `DATABASE_URL`. If you want MySQL specifically, configure the `MYSQL_*` environment variables.
...
- `DATABASE_URL`: a persistent database URL provided by Render or your database host.
- MySQL variables: `MYSQL_DATABASE`, `MYSQL_USER`, `MYSQL_PASSWORD`, `MYSQL_HOST`, `MYSQL_PORT`.

If Render already has a non-MySQL `DATABASE_URL`, the app will use it as the persistent database. When `MYSQL_DATABASE` is present, the app uses the `MYSQL_*` variables instead.
...
If no persistent database is configured on Render, deployment stops intentionally. This prevents users, departments, subjects, and classrooms from being stored in temporary SQLite storage and disappearing after redeploys.
