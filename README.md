...
Use the included `render.yaml` blueprint when creating the Render service. For production, use the existing Render database configuration through `DATABASE_URL` or configure the `MYSQL_*` environment variables.

If the web service is created manually in Render, create or connect a database and add one of these configurations:

- `DATABASE_URL`: the database URL provided by Render or your database host.
- MySQL variables: `MYSQL_DATABASE`, `MYSQL_USER`, `MYSQL_PASSWORD`, `MYSQL_HOST`, `MYSQL_PORT`.

When `MYSQL_DATABASE` is present, the app uses the `MYSQL_*` variables instead of `DATABASE_URL`.
...
Do not store permanent production data in temporary files or browser storage. Render should use the configured database service for users, departments, subjects, and classrooms.
