    ...
    return (
        <React.Fragment>
            ...
            <div className={`fixed inset-y-0 left-0 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0 transition duration-300 ease-in-out w-72 bg-white border-r border-gray-200 flex flex-col h-screen shadow-xl lg:shadow-sm z-50 overflow-hidden`}>
                ...
                <div className="p-4 lg:p-8 flex flex-col items-center space-y-2 lg:space-y-4 relative">
                    <button
                        onClick={() => setIsOpen(false)}
                    ...
                    />
                    <span className="text-xl lg:text-2xl font-bold text-gray-800 tracking-tight leading-tight">KAHE CMS</span>
                </div>

                <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
                    <p className="px-4 text-[10px] lg:text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 lg:mb-4">Main Menu</p>
                    {menuItems.map((item) => (
                        <Link
                            key={item.name}
                            to={item.path}
                            onClick={() => setIsOpen(false)}
                            className={`flex items-center space-x-3 px-4 py-2.5 lg:py-3 rounded-xl transition duration-200 group ${
                                location.pathname === item.path
                                    ? 'bg-indigo-50 text-indigo-700'
                                    : 'text-gray-600 hover:bg-gray-50 hover:text-indigo-600'
                            }`}
                        >
                            <svg className={`h-5 w-5 lg:h-6 lg:w-6 ${location.pathname === item.path ? 'text-indigo-600' : 'text-gray-400 group-hover:text-indigo-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="p-4 lg:p-6 border-t border-gray-100 bg-white">
                    <div className="mb-3 lg:mb-4 px-2">
                        <p className="text-[8px] lg:text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Authenticated</p>
                        <div className="flex items-center space-x-2 lg:space-x-3">
                            <div className={`h-8 w-8 lg:h-10 lg:w-10 text-white rounded-lg lg:rounded-xl flex items-center justify-center font-black text-sm lg:text-lg ${
                                role === 'admin' ? 'bg-green-600' : 'bg-violet-600'
                            }`}>
                                {role?.charAt(0).toUpperCase()}
                            </div>
                            <div className="min-w-0">
                                <p className="text-xs lg:text-sm font-black text-gray-900 leading-tight truncate w-24 lg:w-32">{role?.toUpperCase()}</p>
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center space-x-2 px-4 py-2.5 lg:py-3.5 text-red-600 bg-red-50 hover:bg-red-100 rounded-xl lg:rounded-2xl transition duration-200 font-black text-[10px] lg:text-xs uppercase tracking-widest"
                    >
