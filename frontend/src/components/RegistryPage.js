                            ...
                            <tr key={row.id || i} className="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td className="p-8 text-right">
                                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        {config.fields && config.fields.length > 0 && (
                                            <>
                                                <button onClick={() => { setEditingRecord(row); setFormData(row); setShowModal(true); setMessage({text:'', type:''}); }} className="p-2 bg-white border border-slate-100 rounded-lg text-indigo-500 shadow-sm"><Edit3 size={14} /></button>
                                                <button onClick={async () => { if(window.confirm('Delete?')) { await API.delete(`${config.endpoint}${row.id}/`); fetchData(true); } }} className="p-2 bg-white border border-slate-100 rounded-lg text-rose-400 shadow-sm"><Trash2 size={14} /></button>
                                            </>
                                        )}
                                    </div>
                                </td>
